...
inline math nablaChiH(size_t numParticle) {
    ...
    for(size_t i = 0; i < m_numParticles; i++) {
        ...
        nKernel = nablaKernel(*posAt(m_currentTimestepSinceWrite, numParticle),
                              *posAt(m_currentTimestepSinceWrite, i),
                              m_radiusParticle);
...
inline Vec3 fMag(size_t numParticle) {
    ...
    for(size_t i = 0; i < m_numParticles; i++) {
        if(i == numParticle) continue;
        phi = *phiAt(i);
        sum = nablaKernel(*posAt(m_currentTimestepSinceWrite, numParticle),
                          *posAt(m_currentTimestepSinceWrite, i),
                          m_radiusParticle);
        result.x += sum.x * phi;
        result.y += sum.y * phi;
